...
class Smarty_CacheResource_Pdo extends Smarty_CacheResource_Custom
{
    ...
    protected $fetchStatements = Array(
        'default' => 'SELECT %2$s
                                                                                    FROM %1$s 
                                                                                    WHERE 1 
                                                                                    AND id          = :id 
    ...
    protected function getFetchStatement($columns, $id, $cache_id = null, $compile_id = null)
    {
        if (! is_null($cache_id) && ! is_null($compile_id)) {
            $query = $this->fetchStatements['withCacheIdAndCompileId'] and $args = Array(
                'id' => $id,
                'cache_id' => $cache_id,
                'compile_id' => $compile_id
            );
        } elseif (is_null($cache_id) && ! is_null($compile_id)) {
            $query = $this->fetchStatements['withCompileId'] and $args = Array(
                'id' => $id,
                'compile_id' => $compile_id
            );
        } elseif (! is_null($cache_id) && is_null($compile_id)) {
            $query = $this->fetchStatements['withCacheId'] and $args = Array(
                'id' => $id,
                'cache_id' => $cache_id
            );
        } else {
            $query = $this->fetchStatements['default'] and $args = Array(
                'id' => $id
            );
        }
