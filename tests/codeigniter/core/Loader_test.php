...
class Loader_test extends CI_TestCase
{
    ...
    public function test_library()
    {
        ...
        $this->assertInstanceOf('CI_Loader', $this->load->library(array(
            $lib
        )));
        ...
        $this->setExpectedException('RuntimeException', 'CI Error: Unable to load the requested class: ' . ucfirst($lib));
        $this->assertInstanceOf('CI_Loader', $this->load->library($lib));
    }
    ...
    public function test_driver()
    {
        ...
        // Test loading as an array.
        $this->assertInstanceOf('CI_Loader', $this->load->driver(array(
            $driver
        )));
        $this->assertTrue(class_exists($class), $class . ' does not exist');
    ...
    public function test_model_subdir()
    {
        ...
        $base = 'CI_Model';
        $subdir = 'cars';
        $this->ci_vfs_create($model, '<?php class ' . $model . ' extends ' . $base . ' { }', $this->ci_app_root, array(
            'models',
            $subdir
        ));
        ...
        $this->setExpectedException('RuntimeException', 'CI Error: The model name you are loading is the name of a resource that is already being used: ' . $obj);
    ...
    public function test_load_view()
    {
        ...
        // Test returning view
        $out = $this->load->view($view, array(
            $var => $value
        ), TRUE);
        $this->assertEquals($content . $value, $out);
        
        // Mock output class
        $output = $this->getMock('CI_Output', array(
            'append_output'
        ));
        ...
        // Test view output
        $this->assertInstanceOf('CI_Loader', $this->load->view($view, array(
            $var => $value
        )));
    }
    ...
    public function test_non_existent_view()
    {
        ...
        $this->load->view('ci_test_nonexistent_view', array(
            'foo' => 'bar'
        ));
    }
    ...
    public function test_vars()
    {
        ...
        $key2 = 'boo';
        $val2 = 'hoo';
        $this->assertInstanceOf('CI_Loader', $this->load->vars(array(
            $key1 => $val1
        )));
    ...
    public function test_helper()
    {
        ...
        // Test bad extension
        $this->setExpectedException('RuntimeException', 'CI Error: Unable to load the requested file: helpers/' . $ext . '_helper.php');
    ...
    public function test_language()
    {
        ...
        $file = 'test';
        $lang = $this->getMock('CI_Lang', array(
            'load'
        ));
    ...
    public function test_packages()
    {
        ...
        $class = ucfirst($lib);
        $this->ci_vfs_create(ucfirst($lib), '<?php class ' . $class . ' { }', $this->ci_app_root, array(
            $dir,
            'libraries'
        ));
    ...
    public function test_initialize()
    {
        ...
        $this->ci_vfs_create(ucfirst($drv), '<?php class ' . $drv_class . ' { }', $this->ci_base_root, array(
            'libraries',
            $subdir
        ));
        ...
        $path = APPPATH . $dir . '/';
        $model = 'Automod';
        $this->ci_vfs_create($model, '<?php class ' . $model . ' { }', $this->ci_app_root, array(
            $dir,
            'models'
        ));
