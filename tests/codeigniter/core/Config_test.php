...
class Config_test extends CI_TestCase
{
    ...
    public function test_site_url()
    {
        ...
        $this->assertEquals($index_page . '/' . $uri . '/' . $uri2, $this->config->site_url(array(
            $uri,
            $uri2
        )));
        ...
        $this->assertEquals($index_page . '?' . $uri, $this->config->site_url($uri));
        $this->assertEquals($index_page . '?0=' . $uri . '&1=' . $uri2, $this->config->site_url(array(
            $uri,
            $uri2
        )));
    ...
    public function test_load()
    {
        ...
        $pkg_dir = 'package';
        $this->ci_vfs_create($file . '.php', '<?php $config = ' . var_export($cfg2, TRUE) . ';', $this->ci_app_root, array(
            $pkg_dir,
            'config'
        ));
        ...
        $this->setExpectedException('RuntimeException', 'CI Error: Your ' . $this->ci_vfs_path('config/' . $file . '.php', APPPATH) . ' file does not appear to contain a valid configuration array.');
    ...
    public function test_load_nonexistent()
    {
        ...
        // Test regular fail
        $file = 'absentia';
        $this->setExpectedException('RuntimeException', 'CI Error: The configuration file ' . $file . '.php does not exist.');
        $this->assertNull($this->config->load($file));
    }
}