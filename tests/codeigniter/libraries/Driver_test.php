...
class Driver_test extends CI_TestCase
{
    ...
    public function set_up()
    {
        ...
        $ldr = $this->getMock('CI_Loader', array(
            $paths
        ));
        $ldr->expects($this->any())
            ->method($paths)
    ...
    public function test_load_driver()
    {
        ...
        $class = 'CI_' . $file;
        $prop = 'called';
        $content = '<?php class ' . $class . ' extends CI_Driver { public $' . $prop . ' = FALSE; ' . 'public function decorate($parent) { $this->' . $prop . ' = TRUE; } }';
        $this->ci_vfs_create($file, $content, $this->ci_base_root, array(
            'libraries',
            $this->name,
            'drivers'
        ));
    ...
    public function test_load_app_driver()
    {
        ...
        $this->ci_vfs_create($file, $content, $this->ci_app_root, array(
            'libraries',
            $this->name,
            'drivers'
        ));
        ...
        $this->setExpectedException('RuntimeException', 'CI Error: Unable to load the requested driver: CI_' . $this->name . '_' . $nodriver);
    ...
    public function test_load_driver_ext()
    {
        ...
        $this->ci_vfs_create($base, $content, $this->ci_base_root, array(
            'libraries',
            $this->name,
            'drivers'
        ));
        ...
        $this->ci_vfs_create($class, $content, $this->ci_app_root, array(
            'libraries',
            $this->name,
            'drivers'
        ));
    ...
    public function test_decorate()
    {
        ...
        $method = 'parent_func';
        $return = 'func return';
        $code = 'class ' . $pclass . ' { public $' . $prop . ' = \'' . $value . '\'; ' . 'public function ' . $method . '() { return \'' . $return . '\'; } }';
        eval($code);
        $parent = new $pclass();
